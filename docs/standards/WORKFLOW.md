...
> Moved to [guides/WORKFLOW.md](guides/WORKFLOW.md).
...
**Ursachen:**
- OpenCV nicht installiert
- Quality-Analyse schlug fehl
...
**Hinweis:** Auto-Auswahl ist optional. Nutzer kann trotzdem manuell markieren.
...
```
C:\Photos\
├── Unsorted\          # Input (wilde Sammlung)
├── Sorted\            # Output (strukturiert nach Datum)
└── Archive\           # Optional (manuell für Rejected)
```
...
C:\Photos\
├── Input\
└── Input\Output\      # ❌ Output in Input → Fehler!
```

### Review-Strategie

1. **Erste Runde**: Nur ⭐ empfohlene Bilder prüfen
   - Schnell durchgehen
   - Bei falscher Empfehlung: `K` auf richtigem Bild
   
2. **Zweite Runde**: Unsichere Gruppen
   - Gruppen mit mehreren ähnlich guten Bildern
   - Manuell vergleichen
   
3. **Dritte Runde**: DELETE-Kontrolle
   - Stichproben bei DELETE-markierten Bildern
   - Versehentliche Markierungen korrigieren

### Performance-Tipps

**Große Sammlungen (>5.000 Bilder):**
- Analyse kann 5-10 Minuten dauern
- Teile in kleinere Batches (z.B. pro Jahr)
- Verwende SSD statt HDD

**Netzwerklaufwerke:**
- Kopiere Input zunächst lokal
- Analysiere lokal
- Verschiebe Output danach ins Netzwerk

**HEIC-Dateien:**
- Benötigen mehr Zeit als JPEG
- pillow-heif muss installiert sein
- OpenCV-Fallback nutzt PIL (langsamer)
...
**Ausgangslage:**
- 500 HEIC-Fotos vom iPhone
- Viele Burst-Shots und ähnliche Fotos
- Ziel: Beste Bilder auswählen und nach Datum sortieren
...
**Dialog:**
- Input: `C:\Users\Chris\iPhone\DCIM`
- Output: `C:\Photos\Sorted\2026`
- [Starten]

**Analyse (~2 Minuten):**
- 10%: Indexiere Bilder... (500 Bilder)
- 30%: Finde Duplikate... (47 Gruppen gefunden)
- 50%: Analysiere Bildeigenschaften...
- 70%: Analysiere Bildqualität... (Quality Analyzer läuft)
- 90%: Wähle beste Bilder aus... (47 Empfehlungen)
- 100%: Analyse abgeschlossen

**Review (~10 Minuten):**
- Linke Liste: 47 Gruppen
- Erste Gruppe: 15 Burst-Shots
  - ⭐ zeigt auf schärfstes Bild (Augen offen)
  - Stimmt → Weiter
- Zweite Gruppe: 3 ähnliche Landschaftsfotos
  - ⭐ zeigt auf mittleres Bild
  - Nutzer bevorzugt linkes → `K` drücken
  - ⭐ wechselt zu linkem Bild
- ...weiter durch alle Gruppen...
- Fortschritt: 100% (alle entschieden)

**Export (~1 Minute):**
- [✓ Fertigstellen & Exportieren]
- Dialog: "47 Bild(er) als KEEP markiert. Fortfahren?"
- [Ja]
- Export läuft...
- Erfolgsmeldung: "✓ 47 Bild(er) exportiert"

**Ergebnis:**

```
C:\Photos\Sorted\2026\
├── 01\
│   ├── 02\        # Neujahrsfeier
│   │   ├── IMG_1001.heic
│   │   ├── IMG_1015.heic
...
**Zeitersparnis:**
- Ohne Photo Cleaner: ~2 Stunden manuelles Sortieren
- Mit Photo Cleaner: ~13 Minuten (Analyse + Review + Export)
- **Effizienz: 9x schneller!**

---

## Zusammenfassung

Der neue Workflow ist:

✅ **Geführt**: Klare 4-Phasen-Struktur  
✅ **Robust**: Validierung & Fehlerbehandlung in jeder Phase  
✅ **Automatisiert**: Auto-Auswahl spart 90% der Arbeit  
✅ **Flexibel**: Manuelle Override jederzeit möglich  
✅ **Strukturiert**: EXIF-basierte Ordnerstruktur  
✅ **Sicher**: Originale bleiben unverändert  

Fokus auf **Zuverlässigkeit** und **Verständlichkeit** – kein Feature-Creep!
